...
mod tests {
    ...
    fn activate_on_a_border_color_option_commits_and_collapses() {
        ...
        assert!(st.dirty);
        assert_eq!(st.settings_cursor(), 9, "cursor returned to the BorderColor row");
    }
    ...
    fn activate_on_an_attached_color_option_commits_and_collapses() {
        ...
        assert_eq!(st.settings_cursor(), 8, "cursor returned to the AttachedColor row");
    }
    ...
    fn attached_color_expands_and_collapses_via_step_right_and_left() {
        let mut st = settings_state();
        st.settings_move_cursor(8); // row 8: AttachedColor
        assert_eq!(st.settings_visible_rows().len(), 14);
        st.settings_step_right();
        assert_eq!(st.settings_visible_rows().len(), 14 + 16, "expanded into 16 options");
        assert_eq!(
        ...
        assert_eq!(st.settings_visible_rows().len(), 14, "collapsed back");
        assert_eq!(st.settings_cursor(), 8, "cursor returned to the AttachedColor row");
    }
    ...
    fn border_color_expands_and_collapses_via_step_right_and_left() {
        let mut st = settings_state();
        st.settings_move_cursor(9); // row 9: BorderColor
        st.settings_step_right();
    ...
    fn c_key_quick_cycles_border_color_without_expanding() {
        let mut st = settings_state();
        st.settings_move_cursor(9); // BorderColor, collapsed
        st.settings_cycle_color();
        assert_eq!(st.border_color, "yellow");
        assert!(st.dirty);
        assert_eq!(st.settings_visible_rows().len(), 14, "stays collapsed");
    ...
    fn expanding_and_collapsing_palette_still_refocuses_correctly_with_other_sections_expanded() {
        ...
        st.settings_move_cursor(-1);
        st.settings_step_left(); // collapse AttachedColor again, back to the 14-row layout
        assert_eq!(st.settings_visible_rows().len(), 14);
        st.settings_move_cursor(5); // AttachedColor(8) -> Palette(13)
    ...
    fn h_on_an_attached_color_option_collapses_without_changing_the_value() {
        ...
        assert_eq!(st.settings_cursor(), 8);
    }
